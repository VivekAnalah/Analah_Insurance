...
function OurPartner() {
  ...
  return (
    <div className="mt-[50px]">
      <div className="topic">
        Our <sapn className="topic2">Partners</sapn>
      </div>

      <div className="relative pt-10 lg:py-20">
        <div
          className="grid lg:grid-cols-5 sm:grid-cols-3 grid-cols-2  lg:justify-center lg:items-center  w-[60%]"
          style={{ margin: "auto" }}
        >
          {our_partner &&
            our_partner.map((icons, i) => (
              <div
                key={i}
                className="grid__partner flex justify-center items-center border border-slate-300 grayscale hover:grayscale-0  hover:scale-110 hover:shadow-md hover:shadow-blue-800 hover:bg-[white] hover:z-10  place-items-center justify-items-center "
              >
                {i === 7 || i === 9 || i === 22 || i === 24 ? (
                  <img
                    alt={`${i + 1}`}
